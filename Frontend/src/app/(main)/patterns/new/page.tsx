...
export default function NewPatternPage() {
  ...
  const [materials, setMaterials] = useState<Material[]>([{ materialName: '', quantity: '' }])
    ...
    try {
      await apiFetch('/Pattern', {
        ...
        body: JSON.stringify({
          ...
          difficultyID, courseID: null, tagIDs: [],
  ...
  return (
    <div style={{ backgroundColor: 'var(--cream)', minHeight: '100vh' }}>
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '60px 48px' }}>
        ...
        <p style={{ fontFamily: 'var(--font-inter)', color: 'var(--text-muted)', marginBottom: '40px' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)}
              placeholder="e.g. Classic Granny Square" style={{
          ...
          <div>
            ...
            <p style={{ fontFamily: 'var(--font-inter)', fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '6px' }}>
              Tip: upload to imgbb.com and paste the direct link here
            </p>
          </div>
          ...
          <div>
            ...
            <select value={difficultyID} onChange={e => setDifficultyID(Number(e.target.value))} style={{
              width: '100%', padding: '12px 14px', backgroundColor: 'white',
              border: '1.5px solid var(--border)', borderRadius: '10px',
              fontFamily: 'var(--font-inter)', fontSize: '0.9rem',
              ...
              {materials.map((mat, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 120px 36px', gap: '8px' }}>
                  <input type="text" placeholder="Material name" value={mat.materialName}
                    onChange={e => updateMaterial(i, 'materialName', e.target.value)} style={{
                  ...
                  <input type="text" placeholder="Quantity" value={mat.quantity}
                    onChange={e => updateMaterial(i, 'quantity', e.target.value)} style={{
                      padding: '10px 14px', backgroundColor: 'white',
                      border: '1.5px solid var(--border)', borderRadius: '10px',
                      fontFamily: 'var(--font-inter)', fontSize: '0.875rem',
                      outline: 'none', boxSizing: 'border-box',
